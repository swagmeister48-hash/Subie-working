...
- `subscribe_email` is now a **single 6-arg overload** `(p_email, p_source, p_session, p_part_id, p_target, p_env)`. The old 5-arg version was dropped (June 2026) because a 5-arg call was AMBIGUOUS (PostgREST: "could not choose the best candidate function"). The helper passes all 6 (incl. `p_env`) — keep doing so.
...
## Frontend notes
- Pages: `/` (catalog, `components/PartsBrowser.tsx`), `/retailers` (`app/retailers/page.tsx`), and server-rendered `/part/[slug]` SEO pages (`app/part/[slug]/page.tsx`).
...
- SEO PART PAGES (live 2026-06): server-rendered `/part/[slug]` (dynamic + ISR revalidate 1h) calls `get_part_page(slug)` → real HTML with H1, **per-model fitment** from the payload `fitment[]` array (`[{model,years}]`), all seller prices + Buy links, canonical, and JSON-LD Product/AggregateOffer. Bad slug → 404. Result-list part names link here; `partSlug(name,id)` in `lib/supabase.ts` mirrors the DB `parts.slug` (get_part_page resolves by trailing id anyway). `cleanName()` collapses doubled whitespace in displayed names/titles.
- SITEMAP + ROBOTS: `/sitemap.xml` (`app/sitemap.ts`, `dynamic="force-dynamic"`) pages `sitemap_slugs` in **1000s** (PostgREST caps RPC results at 1000 rows regardless of p_limit) and wraps the walk in `unstable_cache` (revalidate 1h); sequential + retries because concurrent bursts caused intermittent empties, and it THROWS rather than serve a static-only sitemap. ~21k part URLs, single file (<50k). `/robots.txt` (`app/robots.ts`) points at it. ⚠️ Must generate at RUNTIME — Vercel's build-time prerender returned empty and shipped a 2-URL sitemap.
- CANONICAL HOST: `SITE_URL` in `lib/site.ts` = `https://www.subiedeal.com` (apex 301s → www; canonical/sitemap/robots all use www so host signals agree). Override via `NEXT_PUBLIC_SITE_URL`.
- ANALYTICS + VERIFICATION: GA4 via `@next/third-parties` `<GoogleAnalytics gaId="G-37BC372TDM" />` in `app/layout.tsx` — independent of the events-table analytics (unchanged). Impact.com verify: `<meta name="impact-site-verification" value="…">` rendered RAW in the layout — MUST keep `value=` (NOT Next metadata helpers, which emit `content=`). Google Search Console file: `public/google9ee4e04651b08084.html`.
- DEALS STRIP (WIP as of 2026-07 — NOT shipped): `components/WeeklyDeals.tsx` (+ PartsBrowser wiring + `.deals-*` CSS) is a homepage "Top deals this week" strip calling fable's `get_weekly_deals(n,p_model,p_chassis,p_year_from,p_year_to,p_category,p_brand)` RPC. It is **UNCOMMITTED in the MVP checkout only** — not on `main`, not deployed. Left untouched per Abe; finish/verify (confirm the RPC is live) + ship deliberately rather than losing it.
- QA queries:
...
- FITMENT PAIRING FIX (June 2026, user-reported by a DB engineer on Reddit): fitments were a cartesian of all models x all years in a title ("2008-2021 WRX / 2009-2017 Forester" gave Forester 2021). Now segment-paired: years bind to models in their own slash-segment (model-only segments inherit the previous segment's years). ALSO: search_parts + get_facet_counts now require model+year to match the SAME fitment row (they were independently ANDed).
- FITMENT PARSER v2 (June 2026): the parser only understood 4-digit years, so the very common 2-digit form ("02-14 WRX", "22-25 WRX", "18-23 Crosstrek") got ZERO fitments (~40% of indexable parts had none). Now: splits segments on BOTH "/" and ","; parses 4-digit ranges/singles AND 2-digit ranges (expandYY: 00-30→20xx, 90-99→19xx, else skip); BRZ patterns include the GR86/GT86/FT86/FR-S twins. Function `rebuild_fitments()` (run via one-off cron with `set statement_timeout='20min'` in the command — function-level timeout does NOT override pg_cron's 2min cap) rebuilt all 552k fitments. Both crawlers updated (crawl-driver v7, crawl-magento v6). Remaining ~190 indexable no-fitment parts with a year are a long tail (abbrev models like RS/LGT/FXT, or pressure ranges like "25-90 psi"); 9,963 are genuinely universal (wheels/bulbs/accessories — correct). GR86 stragglers heal on the next nightly crawl.

- EMAIL CAPTURE + SEO (June 2026): the email-capture FRONTEND is BUILT + PUSHED (popup / footer / per-part alerts — see "Email capture / price alerts" above). The SEO part-pages + sitemap are now BUILT + PUSHED + LIVE too (see "SEO PART PAGES" / "SITEMAP + ROBOTS" in Frontend notes). Backend:
  ...
  - DONE: email popup/footer/per-part-alert capture; server-rendered /part/[slug] pages (get_part_page); /sitemap.xml from sitemap_slugs + robots.txt; GA4 + Impact + GSC verification. Frontend TODO remaining: price-drop digest email *display* is N/A (backend sends the emails); no open SEO frontend items.

- CRON TIMEOUT — REAL FIX (June 2026, recurred): refresh_part_stats() grew past pg_cron's ~2min worker cap. CRITICAL: `alter function ... set statement_timeout` does NOT override pg_cron's cap — the ONLY fix is putting `set statement_timeout='20min';` IN the cron job command itself. The `refresh-search-stats` job (08:30 UTC) silently failed every night June 9–13 (crawls kept succeeding, so listings stayed fresh but part_stats/price_history froze → site showed stale prices). Fixed via `cron.alter_job(... command := 'set statement_timeout=''20min''; select refresh_part_stats(); select refresh_facets();')`. Refresh takes ~3min now (14 sellers). FIRST DIAGNOSTIC for "prices wrong/stale": check cron.job_run_details for refresh-search-stats failures + compare max(recorded_on) in price_history vs today.

- PRICE-ALERT EMAILS (LIVE June 2026 — sending for real): subscribers.env tags owner/localhost signups 'dev'. price_watches gained last_alerted_price/at. due_price_alerts() returns watches whose best price dropped below signup/last-alert price (and target if set). Edge fn `send-price-alerts` (gated by INGEST_KEY) groups by subscriber, sends Resend digest, marks alerted; supports `test_to` (single sample email) and `dry_run`. Edge fn `unsubscribe?token=` flips unsubscribed. Cron `nightly-price-alerts` 09:00 UTC. Resend domain subiedeal.com VERIFIED, DMARC valid; RESEND_API_KEY stored in private `app_secrets` table (RLS-locked, read by edge fns via service role — NOT in function source). FROM = alerts@subiedeal.com. End-to-end verified (real email delivered). New-domain warmup = early sends may land in spam; self-corrects.

## Conventions
- **LIVE-DB SAFETY (incident 2026-06-08): NEVER run a long table-locking op on a table the live site reads.** `rebuild_fitments()` used to `TRUNCATE fitments` + 17-min insert in one txn — TRUNCATE's ACCESS EXCLUSIVE lock blocked every `search_parts` read for ~17min and a user hit the dead site (console errors, all browsers). Fixed: rebuild_fitments now builds a `fitments_stage` table (reads parts only, no lock on live fitments) then atomically rename-swaps (sub-ms lock). Same rule for any future big rebuild: build-aside-then-swap, or do it off-hours. Heavy maintenance ≠ run-it-on-prod-at-peak.
