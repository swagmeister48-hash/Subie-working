...
- EMAIL CAPTURE + SEO (June 2026): the email-capture FRONTEND is now BUILT + PUSHED (popup / footer / per-part alerts — see "Email capture / price alerts" above). The SEO part-pages + sitemap are still TODO. Backend:
  ...
  - Frontend TODO (remaining, on Abe's word): server-rendered /part/[slug] pages calling get_part_page (the SEO unlock — site is currently one client-rendered page Google can't index); /sitemap.xml from sitemap_slugs; price-drop digest email sender (separate, later). DONE: email popup/footer/per-part-alert capture calling subscribe_email.
